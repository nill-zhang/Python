...
import re,os,sys,shutil,collections,pprint
import logging
...
def del_onetype_files(src,regex,parent=None):
    if parent:
        for eachfile in os.walk(os.path.abspath(src)).next()[2]:
            if re.compile(r'(\.'+regex+')$',re.I)\
           .search(eachfile):
                logging.debug (os.path.join(os.walk(os.path.abspath(src)).next()[0],"\033[92m{}\033[0m".\
                       format(eachfile)))
...
def count_file_onextension(directory):
    if not os.path.exists(os.path.abspath(directory)):
        ...
        for path,dirs,files in os.walk(os.path.abspath(directory)):
            counter.update([os.path.splitext(file)[1] for file in files])
